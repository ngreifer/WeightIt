...
test_that("vcov arg works in vcov(), summary(), and anova() for coxph_weightit", {
  skip_on_cran()
  ...
  test_data$event <- as.numeric(test_data$Y_S < quantile(test_data$Y_S, .8))
  test_data$time <- pmin(test_data$Y_S, quantile(test_data$Y_S, .8))

  W <- weightit(A ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9,
                data = test_data, method = "glm", estimand = "ATE")

  fit_none <- coxph_weightit(survival::Surv(time, event) ~ A * (X1),
                             data = test_data, weightit = W, vcov = "none")
  fit_asympt <- coxph_weightit(survival::Surv(time, event) ~ A * (X1),
                               data = test_data, weightit = W, vcov = "asympt")
  fit_hc0 <- coxph_weightit(survival::Surv(time, event) ~ A * (X1),
                            data = test_data, weightit = W, vcov = "HC0")
  set.seed(123)
  ...
  fit_asympt_clus <- coxph_weightit(survival::Surv(time, event) ~ A * (X1),
                                    data = test_data, weightit = W, vcov = "asympt",
                                    cluster = ~clus)
  fit_hc0_clus <- coxph_weightit(survival::Surv(time, event) ~ A * (X1),
                                 data = test_data, weightit = W, vcov = "HC0",
                                 cluster = ~clus)
  set.seed(123)
  ...
  fit_small <- coxph_weightit(survival::Surv(time, event) ~ A,
                              data = test_data, weightit = W, vcov = "none")
  ...
  fit_small_hc0 <- coxph_weightit(survival::Surv(time, event) ~ A,
                                  data = test_data, weightit = W, vcov = "HC0")
