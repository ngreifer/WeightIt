...
ow.cvxr.fit <- function(A, X, tols = 0, targets, obj = "l2", focal = NULL,
  ...
  #Target constraints
  X <- sweep(X, 2L, targets)
  const_target <- lapply(g2w, function(i) {
    abs(t(X[A == i,]) %*% (S[A_ == i] * W[A_ == i]) / sum(S[A_ == i])) <= tols
  })

  #Objective
  objective <- switch(obj,
                      # "l2" = CVXR::Minimize(CVXR::p_norm(S*(W - B), 2)),
                      "l2" = CVXR::Minimize(sum(S * (W - B)^2)),
                      # "l1" = CVXR::Minimize(CVXR::p_norm(S*(W - B), 1)),
                      "l1" = CVXR::Minimize(sum(S * abs(W - B)) + 1e-9 * sum(S * (W - B)^2)),
                      "linf" = CVXR::Minimize(CVXR::p_norm(S*(W - B), Inf)),
                      "entropy" = CVXR::Minimize(sum(S*CVXR::kl_div(W, B))),
                      # "entropy" = CVXR::Minimize(sum(S * W * log(W /))),
                      "log" = CVXR::Minimize(-sum(S*log(W))), #npCBPS
                      "logit" = CVXR::Minimize(sum(S*((B - exp(-B) - W)))),
                      "DS1" = CVXR::Minimize(sum(S*(W - B)^2/B)/2),
                      "DS2" = CVXR::Minimize(sum(S*(-CVXR::entr(W) - W * log(B) - W + B))),
                      "DS3" = CVXR::Minimize(sum(S*(W - 2 * sqrt(W * B) + B))),
                      "DS4" = CVXR::Minimize(sum(S*(-B * log(W) -CVXR::entr(B) + W - B))),
                      "DS5" = CVXR::Minimize(sum(S*(W - 2*B + (B^2) * CVXR::inv_pos(W)))))
  ...
  res <- CVXR::psolve(prob, ignore_dcp = TRUE,
                      abstol = 1e-7, reltol = 1e-7, num_iter = 1e6)
