...
Finally, we can estimate the effect of the treatment usng a weighted outcome, accounting for estimation of the weights in the standard error of the effect estimate:
...
In addition, `WeightIt` implements the subgroup balancing propensity score using the function `sbps()` and the functions `lm_weightit()` and `glm_weightit()` for fitting regression models after weighting. Several other tools and utilities are available, including `trim()` to trim or truncate weights.
