...
In addition, *WeightIt* implements the subgroup balancing propensity score using the function `sbps()`. Censoring weights can be estimated to account for missingness in the outcome. Several other tools and utilities are available, including `trim()` to trim or truncate weights, `calibrate()` to calibrate propensity scores, and `get_w_from_ps()` to compute weights from propensity scores.
