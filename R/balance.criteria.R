...
init_r2 <- function(covs, treat, s.weights = NULL, poly = 1, int = FALSE, ...) {
  ...
  x <- cbind(`(Intercept)` = 1, covs, int.poly.f(covs, poly = poly, int = int, ex = bin.vars))
...
init_distance.cov <- function(covs, treat, s.weights = NULL, ...) {
  ...
  dist.covs <- transform_covariates(data = covs, method = "scaled_euclidean",
                                    s.weights = s.weights,
                                    var = col.w.v(covs, s.weights, bin.vars))
  Xdist <- unname(eucdist_internal(dist.covs))
...
check_init <- function(init, init_class) {
  ...
  if (missing(init_class)) stop("'init_class' must be specified.")
  if (!inherits(init, init_class)) stop(sprintf("'init' must be of class \"%s\".", init_class))
}
