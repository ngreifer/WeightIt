...
#'
#' @seealso
#' [lm()] and [glm()] for fitting generalized linear models without adjusting standard errors for estimation of the weights.
#'
#' @examples
#'
...
glm_weightit <- function(formula, data, family = gaussian, weightit,
  ...
  if (is_not_null(V)) {
    fit$vcov <- V[seq_len(sum(!aliased)), seq_len(sum(!aliased))]
...
summary.glm_weightit <- function(object, ci = FALSE, level = .95, ...) {
  ...
  if (ci) {
    coef.table <- cbind(coef.table, confint(object, parm = which(!aliased), level = level))
  }
...
print.summary.glm_weightit <- function(x, digits = max(3L, getOption("digits") - 3L),
  ...
  printCoefmat(coefs, digits = digits, signif.legend = FALSE,
               ...
               ...)
  # cat("\n")
  if (x$vcov_type == "const") {
    cat(italic("Standard error: maximum likelihood"))
  }
  else if (x$vcov_type == "asympt") {
    if (is_null(x$cluster))
      cat(italic("Standard error: HC0 robust (adjusted for estimation of weights)"))
    else
      cat(italic("Standard error: HC0 cluster-robust (adjusted for estimation of weights)"))
  }
  else if (x$vcov_type == "HC0") {
    if (is_null(x$cluster))
      cat(italic("Standard error: HC0 robust"))
    else
      cat(italic("Standard error: HC0 cluster-robust"))
  }
  else if (x$vcov_type == "BS") {
    if (is_null(x$cluster))
      cat(italic("Standard error: traditional bootstrap"))
    else
      cat(italic("Standard error: traditional cluster bootstrap"))
  }
  else if (x$vcov_type == "FWB") {
    if (is_null(x$cluster))
      cat(italic("Standard error: fractional weighted bootstrap"))
    else
      cat(italic("Standard error: fractional weighted cluster bootstrap"))
  }
  cat("\n")
  invisible(x)
}
