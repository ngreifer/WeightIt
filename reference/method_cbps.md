...
rather than assuming it is exact. With `include.obj = TRUE`, the `value`
component of the returned
[`optim()`](https://rdrr.io/r/stats/optim.html) output object is the
norm of the mean estimating function, which is what the warning is based
on: it is essentially 0 when the conditions were solved.
