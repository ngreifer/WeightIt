...
Units often drop out of a study before the outcome is measured, and if they do so for reasons related to the covariates, restricting the analysis to those who remain introduces bias in the same way confounding does. `WeightIt` can estimate inverse probability of censoring weights (IPCW) to adjust for this, by wrapping a censoring indicator in `.cens()` on the left side of a model formula. The indicator should be 1 for units that are censored and 0 for those still under observation.
...
set.seed(2000)
msmdata2 <- msmdata
msmdata2$C_2 <- rbinom(nrow(msmdata2), 1,
                       prob = plogis(-2 + .2 * msmdata2$X1_1))

# Post-censoring variables are unobserved for censored units
cens <- msmdata2$C_2 == 1
msmdata2[cens, c("X1_2", "X2_2", "A_3")] <- NA
```
