...
Let's break down the call to `avg_comparisons()`: to the first argument, we supply the model fit, `fit`; to the `variables` argument, the name of the treatment (`"A"`); and to the `vcov` argument, the name of the robust SE we want to request (in this case, HC3, which perform well relative to other robust SEs). Some of these arguments differ depending on the specifics of the outcome type; see the sections below for information.
...
When weighting for an estimand that changes the target population away from one naturally defined by the data, e.g., for the ATO, ATM, or ATOS, or when trimming weights in such a way that trimmed units are dropped from the sample (i.e., receive a weight of 0), an adjustment needs to be made to the call to `avg_comparisons()` and `avg_predictions()`. For the estimands listed, we need to supply the name of the estimated weights to the `wts` argument. For example, after weighting for the ATO, we need to specify the following:

``` r
wts = "weights"
```

To restrict the analysis to just the units with a nonzero weight (e.g., after using `trim(., drop = TRUE)`), we need to supply a version of the dataset with just those units:

``` r
newdata = subset(d, weights != 0)
```
...
You might notice that the standard errors computed using `survey` are different from those using `lm()` or `glm()` and HC3 robust SEs; to get similar SEs using `lm()` or `glm()`, set `vcov = "HC0"` in the call to `avg_comparisons()`. This approach is not recommended because HC3 standard errors tend to perform better, which is one reason to avoid using `survey`. In many cases, though, the results will be similar.

##### Using sampling weights

When sampling weights are required to generalize to the correct population, they must be included in the call to `weightit()` to estimate the balancing weights, in the outcome model, and in the call to `avg_comparisons()`, etc. The balancing weights and sampling weights must be multiplied together prior to inclusion in the call to `svydesign()`, but only the sampling weights are to be supplied to the `wts` argument[^6]. That is, the calls will look like the following:

[^6]: It is not sufficient to supply the name of the weights variable; one must supply the vector of sampling weights directly. This can be a bit complicated for estimands other than the ATE. For the ATT or ATC, the weights should be subset to include only the treated or control units, respectively. For other estimands, the combined weights can be supplied (i.e., the product of the estimated weights and the sampling weights, which are the same weights supplied to `svydesign()`).
...
#Declare a survey design using the combined weights
des <- svydesign(~1, weights = ~weights, data = d)
...
This correctly incorporates the sampling weights at all steps.
